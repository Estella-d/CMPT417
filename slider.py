...
class Board:
    ...
    def init_state(self, patt,goal):
        self.make_goal2(goal)
        b = self.clone_board()
        arr = patt.split(",")
    ...
    def manhattan(self):
        ...
        for i in range(SIZE):
            for j in range(SIZE):
                suma += min_dist(int(self.board[i][j]), i , j)
        return int(suma / 4)
    ...
    def make_goal2(self, pat):
        arr = pat.split(",")
        for i in range(len(arr)):
            row = int(i)//4
            mod = int(i) % 4
            self.board[row][mod] = arr[i]

    def check_goal(self):
        num = 0
        for i in range(SIZE):
            for j in range(SIZE):
                if int(self.board[i][j]) != num:
                    return False
                num += 1
        return True
...
class AbstractBoard(Board):
    ...
    def make_target_pattern(self):
        ...
        for i in range(SIZE):
            for j in range(SIZE):
                if num not in self.distinguish:
                    self.board[i][j] = 'X'
                else:
                    self.board[i][j] = num
                num += 1
    ...
    def check_goal(self):
        ...
        for i in range(SIZE):
            for j in range(SIZE):
                if num in self.distinguish and int(self.board[i][j]) != num:
                    return False
                num += 1
        return True
